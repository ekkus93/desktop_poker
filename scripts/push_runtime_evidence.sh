...
# Finish rustfmt changes and make commit failure rollback the controller.
...
start = handlers.index("    let before_state = authoritative_state", handlers.index("pub(crate) fn handle_action_submission_request"))
...
# Keep the nested HostRuntimeHealth DTO and debug UI in sync with Rust.
replace_once(
    "src/api/desktop.ts",
...
# Apply the same frame cap to outbound and inbound payloads.
replace_once(
...
# Add focused warning tests for both admission counters.
...
# Remove command-boundary substring classification in favor of explicit codes.
...
'''
commands, replacement_count = old_impl.subn(new_impl, commands, count=1)
...
if "observer_action_error_code_is_typed_not_message_parsed" not in commands:
    ...
    commands = commands[:-3] + command_tests + "\n}\n"
commands_path.write_text(commands, encoding="utf-8")

# Keep an explicit review map for hostile input coverage.
...
# Actions recognizes the skip directive for push-triggered workflows. The
# one-time hardening commit intentionally runs CI on its resulting source tree.
