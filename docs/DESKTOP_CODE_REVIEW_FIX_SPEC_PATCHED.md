...
## 14. Copilot Clarifications — Implementation Authority and Exact Targets

This section answers Copilot's follow-up questions. Treat this section as binding for the cleanup pass.

### 14.1 Precedence rule

`DESKTOP_CODE_REVIEW_FIX_SPEC.md` is canonical **only for the desktop cleanup pass**.

It overrides:
- older desktop code-review fix docs
- older desktop cleanup TODOs
- older desktop notes that conflict with this cleanup pass

It does **not** override Android compatibility truth.

For compatibility-sensitive behavior, the source-of-truth order is:
...
Concrete meaning:
- If the cleanup spec says "fix snapshots," implement the fix.
- If the question is "what exact envelope field or QR payload format must desktop use to match Android," follow Android runtime code and `ANDROID_DESKTOP_COMPAT_ANSWERS.md`.
- Do not invent desktop-only protocol behavior unless deliberately versioned and documented for both Android and desktop.

### 14.2 How literally to follow the TODO

`DESKTOP_CODE_REVIEW_FIX_TODO.md` is an ordered implementation checklist, not a shell script.

Copilot must:
- implement the milestones in order unless a dependency forces a small local reordering
- perform the audit/discovery tasks listed in the TODO
- verify actual code paths before deleting or rewriting them
- preserve production behavior that is already correct
- add tests for every fixed defect

Copilot must not:
- skip audit tasks because they are not code changes
- blindly delete code without checking whether it is still referenced
- reinterpret hard product decisions as optional
- use demo/shell paths as substitutes for real production behavior

### 14.3 Exact code path: incomplete client public-event consumption

The defect is in the desktop client session layer.
...
Current problem:
- `ClientRuntimeEvent::Snapshot` updates `latest_snapshot`
- reconnect/error/disconnect states are handled
- but the following are ignored:
  - `ClientRuntimeEvent::PublicEvent { .. }`
  - `ClientRuntimeEvent::PrivateHoleCards(_)`
  - `ClientRuntimeEvent::ResyncRequested { .. }`

The ignored branch is the concrete evidence of incomplete client public-event consumption.

Related emitting code:
- `src-tauri/src/networking/runtime.rs`
- `ClientRuntimeEvent::PublicEvent` is emitted by the client runtime when signed public host events are received
- `ClientRuntimeEvent::PrivateHoleCards` is emitted for private card delivery
- `ClientRuntimeEvent::ResyncRequested` is emitted for resync needs

Required fix:
- `DesktopClientSession::apply_event(...)` must apply public events to client-side session truth.
- It must apply private hole-card events to recipient-private state.
- It must handle resync requests by sending or triggering a real resync path.
- It must not ignore those variants.

### 14.4 Exact code path: misleading event-feed UI

The UI implies a live event feed exists, but production table views are currently fed empty event lists.

Frontend targets:
- `src/screens/MainTableScreen.tsx`
  - renders `Latest public events`
  - reads `tableView.eventFeed`
- `src/screens/HandHistoryScreen.tsx`
  - also reads/renders `tableView.eventFeed`

Backend targets:
- `src-tauri/src/app_state/mod.rs`
  - `DesktopHostSession::table_view(...)` calls `build_table_view_snapshot(..., Vec::new())`
  - `DesktopClientSession::table_view(...)` also calls `build_table_view_snapshot(..., Vec::new())`
  - `build_table_view_snapshot(...)` accepts `event_feed: Vec<TableEventView>` and puts it into `TableViewSnapshot`
- `src-tauri/src/commands.rs`
  - fallback/no-session table view construction also uses `event_feed: vec![]`

Required fix:
- Either feed `TableViewSnapshot.event_feed` from real production public events,
- or remove/hide the event-feed UI from production screens.
- Do not leave a production panel called "Latest public events" that always receives an empty placeholder list.

### 14.5 Exact code path: stale synthetic invite-generation path

The stale invite command is:
...
Current problem:
- `create_host_invite_inner(...)` generates a `JoinPayload` from `DesktopBootstrapState`, not from a real live host session.
- It fabricates:
...
That is not a valid production live-host invite.
...
- frontend/tests may still call `createHostInvite(...)`

Correct production invite source:
- `src-tauri/src/app_state/mod.rs`
- `DesktopHostSession::status(...)`
- `invite: self.host_server.encoded_join_payload().to_string()`

Required fix:
- Production UI must use the live invite from `HostSessionStatus.invite`.
- Remove `create_host_invite` from production command registration, or rename/gate it as debug-only/internal-only.
- If the command remains for tests/dev tools, it must not be reachable from production UI and must be named honestly.

### 14.6 Ready Room direction

This decision is fixed:

- Remove `ReadyRoomScreen` from the production flow.
- Ready-state behavior belongs in the Tournament Lobby.
- Do not repair Ready Room into a separate production screen during this pass unless explicitly asked later.

Targets:
- `src/screens/ReadyRoomScreen.tsx`
- `src/app/AppShell.tsx`
  - remove production route to `/ready-room`
- `src/components/layout/AppFrame.tsx`
  - remove `/ready-room` from production tournament navigation state if present
- `src/test/appIntegrationFixtures.ts`
  - remove or mark route as debug/probe-only
- `src/probe/LayoutProbeApp.tsx`
  - may keep Ready Room only as probe/internal if clearly separated
- tests for `ReadyRoomScreen` should be removed, converted to lobby tests, or marked debug/probe-only

Required result:
- normal production navigation must not route users through `/ready-room`.
- start/readiness flow must happen in the real Tournament Lobby backed by backend state.

### 14.7 Summary of exact production targets

Copilot should focus on these files first:

1. `src-tauri/src/app_state/mod.rs`
   - `DesktopClientSession::apply_event`
   - `DesktopHostSession::table_view`
   - `DesktopClientSession::table_view`
   - `build_table_view_snapshot`
   - any snapshot/private projection helpers

2. `src-tauri/src/networking/runtime.rs`
   - event emission and snapshot/private payload send paths

3. `src-tauri/src/commands.rs`
   - `create_host_invite`
   - `create_host_invite_inner`
   - fallback table view event feed behavior

4. `src-tauri/src/lib.rs`
   - production command registration

5. `src/screens/MainTableScreen.tsx`
   - event feed UI

6. `src/screens/HandHistoryScreen.tsx`
   - event feed/history assumptions

7. `src/screens/ReadyRoomScreen.tsx`
   - remove from production flow or quarantine as internal/probe

8. `src/app/AppShell.tsx`
   - remove production route to Ready Room

9. `src/app/shell.ts`
   - remove production dependency on `buildParticipantShell(...)`

10. `src/api/desktop.ts`
    - remove or gate `createHostInvite(...)` if not production-valid
