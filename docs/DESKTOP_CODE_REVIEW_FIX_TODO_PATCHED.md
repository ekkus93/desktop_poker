...
Required changes:
- [ ] Stop ignoring `ClientRuntimeEvent::PublicEvent`.
- [ ] Stop ignoring `ClientRuntimeEvent::PrivateHoleCards`.
- [ ] Stop ignoring `ClientRuntimeEvent::ResyncRequested`.
- [ ] Apply public events to the client's local session truth.
- [ ] Apply private card events only to the intended recipient's private state.
- [ ] Trigger real resync behavior when `ResyncRequested` is received.
- [ ] Add tests proving public events change client table state without requiring a fresh snapshot.
...
- [ ] If you can wire real production event data now, keep the UI and feed it real events.
...
Required changes:
- [ ] Remove `create_host_invite` from production command registration in `src-tauri/src/lib.rs`, or gate/rename it debug-only.
- [ ] Remove production frontend usage of `createHostInvite(...)` in `src/api/desktop.ts` and related screens.
- [ ] Ensure production host invite copying uses `HostSessionStatus.invite`.
- [ ] Update tests that currently expect synthetic invite generation.
...
Required changes:
- [ ] Remove production route `/ready-room` from `src/app/AppShell.tsx`.
- [ ] Remove `/ready-room` from production tournament navigation logic in `src/components/layout/AppFrame.tsx`.
- [ ] Remove links/buttons that route normal users to `/ready-room`.
- [ ] Delete `src/screens/ReadyRoomScreen.tsx`, or move it to an explicitly debug/probe-only location.
- [ ] Remove/replace `src/screens/ReadyRoomScreen.test.tsx`.
- [ ] Convert any needed readiness assertions into Tournament Lobby tests.
- [ ] Keep `src/probe/LayoutProbeApp.tsx` usage only if clearly probe/internal.
...
Required changes:
- [ ] Remove production route dependencies on `buildParticipantShell(...)`.
- [ ] If it remains for probes/tests, rename or document it as demo/probe-only.
- [ ] Do not let production screens use shell-generated participant/seat state.

### 12.8 Required final checks

Before marking this pass complete:
- [ ] grep for `createHostInvite` and verify no production UI path uses it.
- [ ] grep for `/ready-room` and verify no production route exposes it.
- [ ] grep for `buildParticipantShell` and verify no production screen depends on it.
- [ ] grep for `Vec::new()` event-feed table-view calls and either replace them with real event feed data or remove the visible UI.
- [ ] verify `DesktopClientSession::apply_event(...)` handles public events, private hole cards, and resync requests.
