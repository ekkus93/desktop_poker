...
describe("ErrorStateScreen", () => {
  ...
  it("surfaces an invalid launch invite as the primary state", async () => {
    ...
    expect((await screen.findAllByText(/invite error: bad payload/i)).length).toBeGreaterThan(0);
  ...
  it("keeps scenario controls out of the normal player flow", async () => {
    ...
    expect((await screen.findAllByText(/reconnecting to the table/i)).length).toBeGreaterThan(0);
  ...
  it("keeps the scenario picker available only for debug review", async () => {
    ...
    expect((await screen.findAllByText(/could not restore this session/i)).length).toBeGreaterThan(0);
  });
  ...
  it("exposes the reconnect recovery path with clear next actions", async () => {
    ...
    expect((await screen.findAllByText(/back at the table/i)).length).toBeGreaterThan(0);
